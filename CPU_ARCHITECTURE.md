...
- **Interrupts** (opcode 6xxx + MMIO): `IRET` is the only interrupt opcode; the handler table, per-source mask, and timer period are MMIO registers at base `0xF00F_0000`. See §13.1 and `MMIO_MAP.md`.

These are unlikely to be generated by a C compiler directly but are available via `__asm` or intrinsic wrappers.

### 13.1 Interrupt model

The CPU supports up to 4 interrupt sources (numbered 0–3). Source 0 is hardwired to a periodic timer; sources 1–3 are reserved for future peripherals. All configuration goes through the MMIO interrupt controller at `0xF00F_0000` — the only interrupt-related opcode is `IRET`.

**Configuration registers** — see `MMIO_MAP.md` for the full table:

- `INT_MASK` (`0xF00F_0000`, 4 bits) — per-source enable; bit N = source N. Reset 0 (all masked). Hardware auto-clears bit N for the dispatched source so the handler cannot re-enter on the same source; `IRET` restores the full 4-bit mask.
- `INT_PENDING` (`0xF00F_0008`, R/O) — live pending bits; bit 0 = `r_timer_interrupt`.
- `INT_VECn` (`0xF00F_0010 + 8*n`, 32 bits) — handler byte address for source N. A vector of 0 disables that source even if its mask bit is set. Reset 0.
- `TIMER_PERIOD` (`0xF00F_0030`, 32 bits) — source-0 period in raw `i_Clk` cycles. Reset `0x000F_FFFF` (≈10.5 ms at 100 MHz). Writing it resets the cycle counter so the new period takes effect immediately.
- `TIMER_COUNT` (`0xF00F_0038`, R/O) — live cycle counter, useful for profiling.

**Saved context slot** — when an interrupt fires, the hardware pushes one 64-bit word onto the stack and `IRET` pops it:

```
[63:43] = 21'b0      (reserved)
[42:39] = INT_MASK   (4-bit per-source enable, restored by IRET)
[38]    = r_zero_flag
[37]    = r_equal_flag
[36]    = r_carry_flag
[35]    = r_overflow_flag
...
[33]    = r_less_flag
[32]    = r_ult_flag
[31:0]  = r_PC       (resume address — interrupted instruction)
```

`r_int_push_wait` keeps the FSM in `OPCODE_REQUEST` until the DDR2 push completes; the handler's first fetch begins on the next cycle. `IRET` uses the same multi-cycle DDR2 read pattern as `RET`.

**Typical use:**
```asm
; Install timer handler (handler addr is 0xF00F_0010 → INT_VEC0)
SETR     R0, timer_handler        ; R0 = handler byte address
SETR     R1, 0xF00F0010           ; R1 = address of INT_VEC0
MEMSET32 R0, R1                   ; *INT_VEC0 = handler

; Set timer period (any cycle count) → 0xF00F_0030
SETR     R0, 0x000FFFFF           ; ≈10.5 ms @ 100 MHz
SETR     R1, 0xF00F0030
MEMSET32 R0, R1

; Unmask source 0 → 0xF00F_0000
SETR     R0, 0x1
SETR     R1, 0xF00F0000
MEMSET32 R0, R1
; ... main program runs; timer fires every period ...

timer_handler:
   ; ... do work; PC + flags + mask are all auto-saved/restored ...
   IRET
```
