...
// rd = reg[7:4] (=st.reg_1), rs = reg[3:0] (=st.reg_2): rd <- rs.
...
task t_set_reg;
   input [31:0] i_value;
   begin
      st.wb.value <= {{32{i_value[31]}}, i_value};
      st.wb.rd <= st.reg_2;
      st.SM <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
      st.PC <= st.PC + 8;
   end
...
// st.PC is the opcode address at task time (RISC-V convention).  The 32-bit
...
task t_lea_pc;
   input [31:0] i_value;
   begin
      st.wb.value <= {32'b0, st.PC + i_value};
      st.wb.rd   <= st.reg_2;
      st.SM              <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
      st.PC              <= st.PC + 8;
   end
...
task t_set_reg_flags;
   begin
      st.wb.value <= {st.flags.zero, st.flags.equal, st.flags.carry, st.flags.overflow, 60'b0};
      st.wb.rd <= st.reg_2;
      st.SM <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
      st.PC <= st.PC + 4;
   end
...
task t_and_reg_value;
   input [31:0] i_value;
   begin
      st.wb.value <= r_reg_port_b & i_value;
      st.wb.rd <= st.reg_2;
      st.SM <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
      st.PC <= st.PC + 8;
   end
...
task t_add_value;
   ...
   begin
      ...
      st.alu_pipe_value    <= hold;
      st.alu_pipe_carry    <= carry;
      ...
      st.alu_pipe_overflow <= (!r_reg_port_b[63] && hold[63]) ? 1'b1 : 1'b0;
      st.alu_pipe_mode     <= 1'b0;     // ARITH
      st.wb.set_zero <= 1'b1;
      st.wb.rd <= st.reg_2;
      st.SM <= ALU_FINISH;
      st.PC <= st.PC + 8;
...
task t_addi;
   ...
   begin
      ...
      st.alu_pipe_value    <= hold;
      st.alu_pipe_carry    <= carry;
      st.alu_pipe_overflow <= (r_reg_port_b[63] == sign_ext_imm[63]) &&
                             (hold[63] != r_reg_port_b[63]) ? 1'b1 : 1'b0;
      st.alu_pipe_mode     <= 1'b0;
      st.wb.set_zero <= 1'b1;
      st.wb.rd <= st.reg_1;   // rd = bits[7:4]
      st.SM <= ALU_FINISH;
      st.PC <= st.PC + 8;
   end
endtask
...
task t_compare_reg_value;
   ...
   begin
      ...
      st.alu_pipe_value <= diff;       // sign flag = diff[63] in ALU_FINISH
      st.alu_pipe_equal <= (r_reg_port_b == s_val) ? 1'b1 : 1'b0;
      st.alu_pipe_less  <= (s_reg < s_val) ? 1'b1 : 1'b0;
      st.alu_pipe_ult   <= (r_reg_port_b < u_val) ? 1'b1 : 1'b0;  // sign-extend then unsigned-compare (matches CMPRR)
      st.alu_pipe_mode  <= 1'b1;        // CMP
      st.SM <= ALU_FINISH;
      st.PC <= st.PC + 8;
   end
endtask
...
task t_subr3;
   ...
   begin
      ...
      st.alu_pipe_value    <= hold;
      st.alu_pipe_carry    <= carry;
      st.alu_pipe_overflow <= (r_reg_port_a[63]&&!r_reg_port_b[63]&&!hold[63])||(!r_reg_port_a[63]&&r_reg_port_b[63]&&hold[63]) ? 1'b1 : 1'b0;
      st.alu_pipe_mode     <= 1'b0;
      st.wb.set_zero <= 1'b1;
      st.wb.rd <= st.reg_dst;
      st.SM <= ALU_FINISH;
...
task t_cmprr3;
   ...
   begin
      ...
      st.alu_pipe_value <= diff;       // sign flag = diff[63] in ALU_FINISH
      st.alu_pipe_equal <= (r_reg_port_a == r_reg_port_b) ? 1'b1 : 1'b0;
      st.alu_pipe_less  <= (s_a < s_b) ? 1'b1 : 1'b0;
      st.alu_pipe_ult   <= (r_reg_port_a < r_reg_port_b) ? 1'b1 : 1'b0;
      st.alu_pipe_mode  <= 1'b1;        // CMP
      st.SM <= ALU_FINISH;
      st.PC <= st.PC + 4;
   end
...
task t_addr3;
   ...
   begin
      ...
      st.alu_pipe_value    <= hold[63:0];
      st.alu_pipe_carry    <= hold[64];
      st.alu_pipe_overflow <= (r_reg_port_a[63] == r_reg_port_b[63]) &&
...
task t_addc3;
   logic [65:0] hold;
   begin
      hold = {1'b0, r_reg_port_a} + {1'b0, r_reg_port_b} + {65'b0, st.flags.carry};
      st.alu_pipe_value    <= hold[63:0];
      st.alu_pipe_carry    <= hold[64];
      st.alu_pipe_overflow <= (r_reg_port_a[63] == r_reg_port_b[63]) &&
                             (hold[63] != r_reg_port_a[63]) ? 1'b1 : 1'b0;
   ...
   begin
      if (st.extra_clock == 0) begin
         ...
         if (w_mem_ready) begin
            st.mem_read_DV     <= 1'b0;
            st.wb.value <= {(st.PC[2] ? w_mem_read_data[63:32] : w_mem_read_data[31:0]), i_lo};
            st.wb.rd   <= st.reg_2;
            st.SM              <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
            st.PC              <= st.PC + 12;  // opcode(4) + lo32(4) + hi32(4)
         end
      end
   end
endtask
...
task t_sextw;
   begin
      st.wb.value <= {{32{r_reg_port_b[31]}}, r_reg_port_b[31:0]};
      st.wb.rd <= st.reg_2;
      st.SM <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
      st.PC <= st.PC + 4;
   end
...
task t_zextw;
   begin
      st.wb.value <= {32'b0, r_reg_port_b[31:0]};
      st.wb.rd <= st.reg_2;
      st.SM <= OPCODE_REQUEST; st.wb.pending <= 1'b1;
      st.PC <= st.PC + 4;
   end
